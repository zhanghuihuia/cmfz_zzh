	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#captchaImage").click(function(){//点击更换验证码
                this.src='${pageContext.request.contextPath}/code/code?d='+Math.random();
			});
			
			//  form 表单提交
			$("#loginForm").form("submit",function(){
			    alert(111);
			    $.post("${pageContext.request.contextPath}/admin/login",function () {

                });
				return false;
			});
			//
			$("#username").textbox({required:true,iconCls:'icon-man',missingMessage:'请输入用户名'});
            $("#password").passwordbox({required:true,missingMessage:'请输入密码'});
            $("#enCode").textbox({required:true,missingMessage:'请输入验证码'});
		});
	</script>
</head>
<body>
				...
				<table>
					<tbody>
						<tr>
							...
							<td>
								<input type="text"  name="username" class="text" id="username" maxlength="20"/>
							</td>
					  </tr>
					  <tr>
							<th>
							...
							<td>
								<input id="password" name="password" class="text" maxlength="20" autocomplete="off"/>
							</td>
					  </tr>
						...
						<tr>
							...
							<th>验证码:</th>
							<td>
								<input id="enCode" name="encode" class="text captcha" maxlength="4" autocomplete="off"/>
